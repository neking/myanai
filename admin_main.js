...
// ── Myanmar Font select change (handles built-in + custom uploaded) ──
// LPE font functions → admin_lpe.js
// ── KPay QR Upload ──────────────────────────────
...
function saasRender(list){
  ...
  tbody.innerHTML = list.map((t,idx)=>{
    ...
    return `<tr data-id="${t.id}" data-idx="${idx}">
      ...
      <td style="color:var(--muted);font-size:.78rem;font-weight:500">${idx+1}</td>
      <td>
...
function renderTenantPager(total,pages){
  let pg=document.getElementById('tenant-pager');
  if(!pg){
    pg=document.createElement('div');pg.id='tenant-pager';
    pg.style.cssText='display:flex;align-items:center;gap:.4rem;padding:.6rem 0;flex-wrap:wrap;font-size:.82rem';
    const tbl=document.getElementById('tenants-tbody');
    if(tbl) tbl.closest('table')?.after(pg);
  }
  if(pages<=1){pg.innerHTML='';return;}
  let html=`<span style="color:var(--muted);margin-right:.3rem">Total: ${total}</span>`;
  html+=`<button onclick="tenantGoPage(1)" style="padding:.2rem .5rem;border:1px solid var(--border);border-radius:5px;background:var(--warm);color:var(--ink);cursor:pointer" ${_tenantPage===1?'disabled':''}>«</button>`;
  html+=`<button onclick="tenantGoPage(${_tenantPage-1})" style="padding:.2rem .5rem;border:1px solid var(--border);border-radius:5px;background:var(--warm);color:var(--ink);cursor:pointer" ${_tenantPage===1?'disabled':''}>‹</button>`;
  for(let p=Math.max(1,_tenantPage-2);p<=Math.min(pages,_tenantPage+2);p++){
    const active=p===_tenantPage;
    html+=`<button onclick="tenantGoPage(${p})" style="padding:.2rem .5rem;border:1px solid ${active?'var(--accent)':'var(--border)'};border-radius:5px;background:${active?'var(--accent)':'var(--warm)'};color:${active?'#fff':'var(--ink)'};cursor:pointer;font-weight:${active?700:400}">${p}</button>`;
  }
  html+=`<button onclick="tenantGoPage(${_tenantPage+1})" style="padding:.2rem .5rem;border:1px solid var(--border);border-radius:5px;background:var(--warm);color:var(--ink);cursor:pointer" ${_tenantPage===pages?'disabled':''}>›</button>`;
  html+=`<button onclick="tenantGoPage(${pages})" style="padding:.2rem .5rem;border:1px solid var(--border);border-radius:5px;background:var(--warm);color:var(--ink);cursor:pointer" ${_tenantPage===pages?'disabled':''}>»</button>`;
  pg.innerHTML=html;
}
function tenantGoPage(p){
  _tenantPage=p;
...
// LPE editor functions (lpeTab, lpeRT, lpeSave etc.) → admin_lpe.js
/* ══ NOTIFICATIONS ══ */
  ...
  setTimeout(async ()=>{
    ...
    setTimeout(()=>{
      ...
      for(const row of rows){
        ...
        if(idCell && idCell.textContent.trim() === String(tenantId)){
          row.scrollIntoView({behavior:'smooth', block:'center'});
          row.style.transition='background .3s';
          row.style.background='var(--accent2)';
...
async function loadGrowthSummary(){
  const elCheck = document.getElementById('growth-summary-grid');
  if(!elCheck) return;
  const d = await fetch('admin_growth.php?months=1',{credentials:'include'}).then(r=>r.json()).catch(()=>({ok:false}));
  if(!d.ok) return;
  const s = d.summary||{};
